## 保证数据长度一致性并保留ACC三轴结构的WESAD数据预处理

# 在此部分，我们通过加载WESAD数据集中的特定受试者数据，并使用线性插值法对齐不同特征的数据长度，以便后续的数据处理和模型训练。此外，保留ACC数据的三轴结构，以避免数据的歧义。
...
def interpolate_data(data, target_length):
    ...
    interpolated_data = np.array([np.interp(x_new, x_old, data[:,i]) for i in range(data.shape[1])]).T
    return interpolated_data

# 数据预处理
def preprocess_data(data):
    target_length = max(len(data['signal']['chest']['ECG']),
                        len(data['signal']['chest']['EDA']),
                        len(data['signal']['chest']['Temp']),
                        len(data['signal']['chest']['ACC']))
    
    ecg_data = interpolate_data(data['signal']['chest']['ECG'], target_length)
    eda_data = interpolate_data(data['signal']['chest']['EDA'].reshape(-1, 1), target_length)
    temp_data = interpolate_data(data['signal']['chest']['Temp'].reshape(-1, 1), target_length)
    acc_data = interpolate_data(data['signal']['chest']['ACC'], target_length)
...
# 加载特定受试者的数据
participant_id = 3  # 修改为其他受试者编号进行测试
