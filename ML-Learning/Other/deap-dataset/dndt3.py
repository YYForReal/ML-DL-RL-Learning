...
# python dndt.py --participant 0 --num_epochs 100 --use_gpu
...
# 训练模型并保存结果
def train_and_evaluate_model(X_train, X_test, y_train, y_test, label_name):
    input_dim = X_train.shape[1]
    num_cut_points = 3  # 可调整
    num_classes = 2
    model = DNDT(input_dim, num_cut_points, num_classes)
    ...
    print("Loss history:", loss_history)  # 打印损失历史
    ...
    # 保存模型和结果
    save_model(model, f"{label_name}_dndt_model.pth")
    with open(
        os.path.join("output", f"{label_name}_classification_report.txt"), "w"
    ) as f:
    ...
    plt.legend()
    plt.savefig(os.path.join("output", f"{label_name}_training_loss.png"))
    # plt.show()
    ...
    for metric in metrics:
        plt.figure()
        plt.bar(
            range(len(report)), [report[str(i)][metric] for i in range(num_classes)]
        )
        plt.xlabel("Classes")
        plt.ylabel(metric.capitalize())
        plt.title(f"{label_name.capitalize()} {metric.capitalize()}")
        plt.savefig(os.path.join("output", f"{label_name}_{metric}.png"))
        # plt.show()
    ...
    plt.savefig(os.path.join("output", f"{label_name}_confusion_matrix.png"))
    # plt.show()
